...
router.post('/', async(req, res) =>{
    const{ productId, userUserName, orderNumber, shippingAddress, status, amount} = req.body
    console.log(req.body)
    try {
        const order = await Order.create({productId, userUserName, orderNumber, shippingAddress, status, amount})
        res.send(order);
    } catch (error) {
        console.log(error)
    }
